...
export class CompletionReplace extends CompletionAbstract implements CompletionInterface {
    ...
    public async complete(
        ...
        // NOWE: obsługa alternatywnej składni REPLACE INTO tbl SET col1 = val1, col2 = val2 (MySQL/MariaDB dopuszcza SET zamiast (columns) VALUES (...))
        ...
        if (allColumnsMatch) {
            ...
            if (schema && tableName) {
                ...
                if (columns.length > 0) {
                    ...
                    // NOWE: alternatywna składnia SET obok listy kolumn - użytkownik może wybrać jedną z dwóch form REPLACE
                    const setKeywordItem = new vscode.CompletionItem('SET', vscode.CompletionItemKind.Keyword);
                    setKeywordItem.detail = 'SQL Keyword (alternative REPLACE ... SET syntax)';
        ...
        const objectMatch = linePrefix.match(REGEX_REPLACE_OBJECT);
        if (objectMatch) {
