...
interface InsertGenerationOption {
    id: string;
    label: string;
    detail: string;
...
const INSERT_GENERATION_OPTIONS: InsertGenerationOption[] = [
...
export class SqlResultsProvider implements vscode.WebviewViewProvider {
    ...
    private resolveInsertOptionGroupSelection(selectedIds: Set<string>, previousSelectedIds: Set<string>): Set<string> {
    ...
    private async pickInsertGenerationOptions(): Promise<{ ids: Set<string>; batchSize?: number } | undefined> {
        const remembered = new Set(
            this._context?.workspaceState.get<string[]>(INSERT_GENERATION_OPTIONS_STATE_KEY, []) ?? []
        );
        ...
        return new Promise<{ ids: Set<string>; batchSize?: number } | undefined>((resolve) => {
            ...
            quickPick.onDidChangeSelection((selection) => {
                ...
                const finalIds = this.resolveInsertOptionGroupSelection(selectedIds, previousSelectedIds);
            ...
            quickPick.onDidAccept(async () => {
                ...
                if (chosenIds.has('batchSize')) {
                    const rememberedBatchSize = this._context?.workspaceState.get<number>(INSERT_GENERATION_BATCH_SIZE_STATE_KEY, DEFAULT_INSERT_BATCH_SIZE);
                    ...
                    if (input === undefined) {
                        ...
                        this._context?.workspaceState.update(INSERT_GENERATION_BATCH_SIZE_STATE_KEY, batchSize);
                    }
                }

                this._context?.workspaceState.update(INSERT_GENERATION_OPTIONS_STATE_KEY, [...chosenIds]);
                quickPick.dispose();
    ...
    private buildInsertStatementPrefix(selectedOptions: Set<string>): string {
    ...
    private buildInsertSql(
        ...
        const prefix = this.buildInsertStatementPrefix(selectedOptions);
    ...
    private async generateInsertSQL(rowKeys: number[]) {
        try {
            ...
            const selection = await this.pickInsertGenerationOptions();
            ...
            const sql = this.buildInsertSql(rows, columns, primaryKeys, qualifiedTable, selectedOptions, batchSize);
